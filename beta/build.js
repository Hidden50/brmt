...
this.compendiumToHtmlImages = function compendiumToHtmlImages (compendium, build, options, species, set, mode, targetSpecies) {
	...
	let tilesMap =         aSet =>     brmtIcon(compendium, species, aSet);
	...
	let targetSetMap =     aSet =>     brmtIcon(compendium, targetSpecies, aSet);
	...
	if (!mode)           return brmtIcon(compendium, species, set) + ` <b>${set}</b><br>` + Object.keys(build[species][set]).map( modeMap ).join('<br>');
...
function brmtIcon (compendium, species, set, cornerDisplay) {  // todo: wraplines, nowraplines, sets
	let image = `<img src="${Weblink(species, set)}">`;
	if (!set) return image;
	if (cornerDisplay) return `<span title="${species} (${set})" class="iconWrapper">${image}<span ${miniIconStyle}>${cornerDisplay}</span></span>`;
	let miniIcon;
	...
	else return brmtIcon( compendium, species, set, `<b ${miniIconFontStyle}>${set}</b>` );
	
	if (miniIcon.letters !== undefined)
		return brmtIcon( compendium, species, set, `<b ${miniIconFontStyle}>${miniIcon.letters}</b>` );
	if (miniIcon.icon)
		return brmtIcon( compendium, species, set, `<img src="${Weblink(miniIcon.icon)}" width=18px height=18px">` );
	if (miniIcon.image)
		return brmtIcon( compendium, miniIcon.image );
}

...
htmloutput.sprite = (imgName) => {
	imgName = brmt.aliases.getSpeciesID(imgName);

	let type;
	if (imgName.match(/^\d/))
		type = "p-icon";  // pokemon image
	else type = "i-icon";  // item image

	return `<span class="${type} sprite-${imgName}"></span>`;
};
...
htmloutput.brmtIcon = function brmtIcon (pokemon, build, team, iconConfig, rating) {
	...
	if (icon)
		icon = htmloutput.sprite(icon);
	else icon = "";
	rating = `<span class='ratingWrapper'>${(rating === undefined) ? "" : rating}</span>`;
	image = htmloutput.sprite(image || species);
